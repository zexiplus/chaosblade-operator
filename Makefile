...
VERSION_PKG=github.com/zexiplus/chaosblade-operator/version
...
docker-build:
	...
	docker buildx build -f build/image/amd/Dockerfile --platform=linux/amd64 -t ghcr.io/zexiplus/chaosblade-operator:${BLADE_VERSION} .

docker-build-arm64:
	GOOS="linux" GOARCH="arm64" go build $(GO_FLAGS) -o build/_output/bin/chaosblade-operator cmd/manager/main.go
	docker buildx build -f build/image/arm/Dockerfile  --platform=linux/arm64  -t ghcr.io/zexiplus/chaosblade-operator-arm64:${BLADE_VERSION} .

push_image:
	docker push ghcr.io/zexiplus/chaosblade-operator:${BLADE_VERSION}
	docker push ghcr.io/zexiplus/chaosblade-operator-arm64:${BLADE_VERSION}

#operator-sdk 0.19.0 build
build_all_operator: pre_build build build_image
build_image:
	operator-sdk build --go-build-args="$(GO_FLAGS)" ghcr.io/zexiplus/chaosblade-operator:${BLADE_VERSION}

build_image_arm64:
	GOOS="linux" GOARCH="arm64" operator-sdk build --go-build-args="$(GO_FLAGS)" ghcr.io/zexiplus/chaosblade-operator-arm64:${BLADE_VERSION}
...
build_linux:
	...
	docker run --rm \
		...
		-v $(shell pwd):/go/src/github.com/zexiplus/chaosblade-operator \
		-w /go/src/github.com/zexiplus/chaosblade-operator \
...
build_arm64:
	...
	docker run --rm \
		...
		-v $(shell pwd):/go/src/github.com/zexiplus/chaosblade-operator \
		-w /go/src/github.com/zexiplus/chaosblade-operator \
		zexiplus/chaosblade-build-arm:latest
